...
const RestTimer: React.FC<RestTimerProps> = ({
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      if (isPaused) {
        // If paused, just schedule next frame without updating time
  ...
  const togglePause = () => {
    if (isPaused) {
      // Resume: add paused time to total paused duration
  ...
  const adjustTime = (adjustment: number) => {
    ...
    // Adjust the start time to account for the time change
    if (startTimeRef.current !== null) {
      startTimeRef.current += adjustment * 1000 // Convert to milliseconds
    }
